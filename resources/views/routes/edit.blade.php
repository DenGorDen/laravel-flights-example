...
<form action="{{ route('airroutes.update',$airroute->id) }}" method="POST" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    <div class="row">
        <div class="col-xs-12 col-sm-12 col-md-12">
            <div class="form-group">
                ...
                <input type="text" name="nameline" value="{{ $airroute->nameline }}" class="form-control" placeholder="**-****">
                @error('nameline')
        ...
        <div class="col-xs-12 col-sm-12 col-md-12">
            <div class="form-group">
                ...
                <select name="startcity" id="">
                    ...
                    @if($city->id==$airroute->startcity) selected="selected" @endif
                     >{{$city->nicName}}</option>
                    @endforeach
                </select>
        ...
        <div class="col-xs-12 col-sm-12 col-md-12">
            <div class="form-group">
                ...
                <select name="finishcity" id="">
                    @foreach ($cites as $city)
                    <option value="{{$city->id}}"
                    @if($city->id==$airroute->finishcity) selected="selected" @endif
                     >{{$city->nicName}}</option>
                    @endforeach
                </select>
                @error('finishcity')
